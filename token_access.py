...
headers = {'X-Application': 'PHJ1DsoXJN2xok3f', 'Content-Type': 'application/x-www-form-urlencoded'}
...
def getMarketBookBestOffers(marketId):
    ...
    market_book_req = '{"jsonrpc": "2.0", "method": "SportsAPING/v1.0/listMarketBook", "params": {"marketIds":["' + marketId + '"],"priceProjection":{"priceData":["EX_ALL_OFFERS"], "exBestOffersOverrides":{"rollupModel":"STAKE","bestPricesDepth":10}}, "virtualise": true, "orderProjection": "ALL", "matchProjection":"NO_ROLLUP","currencyCode":"EUR"}}, "id": 1}'
    """
...
#Adrew
headers2 = {'X-Application': 'PHJ1DsoXJN2xok3f','X-Authentication': resp_json['sessionToken'], 'Content-Type': 'application/x-www-form-urlencoded'}
...
try:
    ...
    for market in marketCatalogueResult:
        ...
        print (market)
        print("***************************************************************************")
        market_book_result = getMarketBookBestOffers(market['marketId'])
        print (market_book_result)
        print("---------------------------------------------------------------------------")
        printPriceInfo(market_book_result)
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        entry = market['marketId'] + '#' + market['marketName'] + '#' + market['marketStartTime']+ '#'+ str(market['totalMatched'])+ '#'
        for runner in market['runners']:
            entry += str(runner['selectionId'])+'#'+runner['runnerName']+'#'
        entry += market['competition']['name']+ '#' + market['event']['id']+ '#' + market['event']['name']+ '#'
        fd.write(entry + "\n")
    fd.close()
except:
